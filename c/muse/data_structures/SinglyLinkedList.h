...
size_t SinglyLinkedList_size(SinglyLinkedList *list) { return list->length; }

bool SinglyLinkedList_isEmpty(SinglyLinkedList *list) {
  return list->length == 0;
}

singly_linked_list_type SinglyLinkedList_get(SinglyLinkedList *list,
                                             size_t index) {
...
void SinglyLinkedList_set(SinglyLinkedList *list, size_t index,
                          singly_linked_list_type element) {
  if (index < 0 || index >= list->length) {
    fprintf(stderr, "[PANIC - IndexOutOfBounds]\n");
    exit(EXIT_FAILURE);
  }
...
void SinglyLinkedList_insert(SinglyLinkedList *list, size_t index,
                             singly_linked_list_type element) {
  if (index < 0 || index > list->length) {
    fprintf(stderr, "[PANIC - IndexOutOfBounds]\n");
    exit(EXIT_FAILURE);
...
void SinglyLinkedList_prepend(SinglyLinkedList *list,
                              singly_linked_list_type element) {
  SinglyLinkedList_insert(list, 0, element);
}

void SinglyLinkedList_append(SinglyLinkedList *list,
                             singly_linked_list_type element) {
