...
def testGreatestCommonDivisor
  ...
  sample.each_index do |i|
    return false if GreatestCommonDivisor.iterativeBinaryGCD(sample[i][0], sample[i][1]) != sample[i][2]
  end

  sample.each_index do |i|
    return false if GreatestCommonDivisor.recursiveBinaryGCD(sample[i][0], sample[i][1]) != sample[i][2]
  end

  sample.each_index do |i|
    return false if GreatestCommonDivisor.iterativeEuclidean(sample[i][0], sample[i][1]) != sample[i][2]
  end

  sample.each_index do |i|
    return false if GreatestCommonDivisor.recursiveEuclidean(sample[i][0], sample[i][1]) != sample[i][2]
  end

  true
end

TestRunner.pick(testGreatestCommonDivisor) if __FILE__ == $PROGRAM_NAME
