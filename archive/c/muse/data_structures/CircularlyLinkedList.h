...
circularly_linked_list_type CircularlyLinkedList_get(CircularlyLinkedList *list,
                                                     size_t index) {
...
void CircularlyLinkedList_set(CircularlyLinkedList *list, size_t index,
                              circularly_linked_list_type element) {
  if (index < 0 || index >= list->length) {
    fprintf(stderr, "[PANIC - IndexOutOfBounds]\n");
    exit(EXIT_FAILURE);
  }
...
void CircularlyLinkedList_insert(CircularlyLinkedList *list, size_t index,
                                 circularly_linked_list_type element) {
  if (index < 0 || index > list->length) {
    fprintf(stderr, "[PANIC - IndexOutOfBounds]\n");
    exit(EXIT_FAILURE);
...
circularly_linked_list_type
CircularlyLinkedList_front(CircularlyLinkedList *list) {
  return CircularlyLinkedList_get(list, 0);
}

circularly_linked_list_type
CircularlyLinkedList_back(CircularlyLinkedList *list) {
  return CircularlyLinkedList_get(list, list->length - 1);
}

void CircularlyLinkedList_prepend(CircularlyLinkedList *list,
                                  circularly_linked_list_type element) {
  CircularlyLinkedList_insert(list, 0, element);
}

void CircularlyLinkedList_append(CircularlyLinkedList *list,
                                 circularly_linked_list_type element) {
