...
ArrayList *ArrayList_new(size_t physicalSize) {
  ...
  list->data =
      (array_list_type *)malloc(list->physicalSize * sizeof(array_list_type));

  return list;
}

size_t ArrayList_size(ArrayList *list) { return list->logicalSize; }

bool ArrayList_isEmpty(ArrayList *list) { return list->logicalSize == 0; }
...
void ArrayList_insert(ArrayList *list, size_t index, array_list_type element) {
  ...
  if (list->logicalSize == list->physicalSize) {
    ...
    array_list_type *temp =
        (array_list_type *)malloc(newCapacity * sizeof(array_list_type));
...
void ArrayList_poll(ArrayList *list) { ArrayList_remove(list, 0); }

void ArrayList_eject(ArrayList *list) {
  ArrayList_remove(list, list->logicalSize - 1);
}

size_t ArrayList_capacity(ArrayList *list) { return list->physicalSize; }

void ArrayList_shrink(ArrayList *list) {
  array_list_type *temp =
      (array_list_type *)malloc(list->logicalSize * sizeof(array_list_type));
