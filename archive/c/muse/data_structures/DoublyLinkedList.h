...
size_t DoublyLinkedList_size(DoublyLinkedList *list) { return list->length; }

bool DoublyLinkedList_isEmpty(DoublyLinkedList *list) {
  return list->length == 0;
}

doubly_linked_list_type DoublyLinkedList_get(DoublyLinkedList *list,
                                             size_t index) {
...
void DoublyLinkedList_set(DoublyLinkedList *list, size_t index,
                          doubly_linked_list_type element) {
  if (index < 0 || index >= list->length) {
    fprintf(stderr, "[PANIC - IndexOutOfBounds]\n");
    exit(EXIT_FAILURE);
  }
...
void DoublyLinkedList_insert(DoublyLinkedList *list, size_t index,
                             doubly_linked_list_type element) {
  if (index < 0 || index > list->length) {
    fprintf(stderr, "[PANIC - IndexOutOfBounds]\n");
    exit(EXIT_FAILURE);
...
void DoublyLinkedList_prepend(DoublyLinkedList *list,
                              doubly_linked_list_type element) {
  DoublyLinkedList_insert(list, 0, element);
}

void DoublyLinkedList_append(DoublyLinkedList *list,
                             doubly_linked_list_type element) {
