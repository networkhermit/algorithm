...
ArrayQueue *ArrayQueue_new(size_t physicalSize) {
  ...
  queue->data = (array_queue_type *)malloc(queue->physicalSize *
                                           sizeof(array_queue_type));

  return queue;
}

size_t ArrayQueue_size(ArrayQueue *queue) { return queue->logicalSize; }

bool ArrayQueue_isEmpty(ArrayQueue *queue) { return queue->logicalSize == 0; }
...
void ArrayQueue_enqueue(ArrayQueue *queue, array_queue_type element) {
  if (queue->logicalSize == queue->physicalSize) {
    ...
    array_queue_type *temp =
        (array_queue_type *)malloc(newCapacity * sizeof(array_queue_type));
  ...
  queue->data[(queue->front + queue->logicalSize) % queue->physicalSize] =
      element;
...
size_t ArrayQueue_capacity(ArrayQueue *queue) { return queue->physicalSize; }

void ArrayQueue_shrink(ArrayQueue *queue) {
  array_queue_type *temp =
      (array_queue_type *)malloc(queue->logicalSize * sizeof(array_queue_type));
