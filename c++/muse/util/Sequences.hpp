...
void inspect(const std::vector<int> &arr) {
  ...
  for (std::size_t i = 0, length = arr.size(); i < length; i++) {
    std::cout << "\t#" << std::setfill('0') << std::setw(4) << std::uppercase
              << std::hex << i << "  -->  " << std::dec << arr[i] << std::endl;
  }
  std::cout << "]" << std::endl;
}
...
void reverse(std::vector<int> &arr) {
  ...
  for (std::size_t i = 0, bound = arr.size() >> 1, length = arr.size();
       i < bound; i++) {
    k = length - i - 1;
    temp = arr[i];
    arr[i] = arr[k];
...
void sort(std::vector<int> &arr) { QuickSort::sort(arr); }
} // namespace Sequences
