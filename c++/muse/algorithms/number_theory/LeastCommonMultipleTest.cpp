...
bool testLeastCommonMultiple() {
  ...
  for (size_t i = 0, size = *(&sample + 1) - sample; i < size; i++) {
    if (LeastCommonMultiple::reduceToBinaryGCD(sample[i][0], sample[i][1]) !=
        sample[i][2]) {
      return false;
    }
  }
  ...
  for (size_t i = 0, size = *(&sample + 1) - sample; i < size; i++) {
    if (LeastCommonMultiple::reduceToEuclidean(sample[i][0], sample[i][1]) !=
        sample[i][2]) {
      return false;
    }
  }
...
int main() { TestRunner::pick(&testLeastCommonMultiple); }
