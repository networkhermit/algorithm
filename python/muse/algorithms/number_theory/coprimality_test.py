...
def test_coprimality() -> bool:
    ...
    for i, _ in enumerate(sample):
        if sample[i][2] == 0:
            if coprimality.reduce_to_binary_gcd(sample[i][0], sample[i][1]):
                return False
        else:
            if not coprimality.reduce_to_binary_gcd(sample[i][0], sample[i][1]):
                return False

    for i, _ in enumerate(sample):
        if sample[i][2] == 0:
            if coprimality.reduce_to_euclidean(sample[i][0], sample[i][1]):
                return False
        else:
            if not coprimality.reduce_to_euclidean(sample[i][0], sample[i][1]):
                return False
