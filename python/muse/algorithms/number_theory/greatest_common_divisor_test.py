...
def derive(fn: Callable[[int, int], int]) -> Callable[[], bool]:
    def f() -> bool:
        for tc in sample:
            if fn(tc[0], tc[1]) != tc[2]:
                return False
        return True
...
def test_greatest_common_divisor() -> bool:
    return (
        derive(greatest_common_divisor.iterative_binary_gcd)()
        and derive(greatest_common_divisor.recursive_binary_gcd)()
        and derive(greatest_common_divisor.iterative_euclidean)()
        and derive(greatest_common_divisor.recursive_euclidean)()
    )


def main() -> None:
    test_runner.pick(test_greatest_common_divisor)
