...
func TestGreatestCommonDivisor(t *testing.T) {
	...
	for i := range sample {
		if IterativeBinaryGCD(sample[i][0], sample[i][1]) != sample[i][2] {
			t.FailNow()
		}
	}

	for i := range sample {
		if RecursiveBinaryGCD(sample[i][0], sample[i][1]) != sample[i][2] {
			t.FailNow()
		}
	}

	for i := range sample {
		if IterativeEuclidean(sample[i][0], sample[i][1]) != sample[i][2] {
			t.FailNow()
		}
	}

	for i := range sample {
		if RecursiveEuclidean(sample[i][0], sample[i][1]) != sample[i][2] {
			t.FailNow()
		}
	}
}
